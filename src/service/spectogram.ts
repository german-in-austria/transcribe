export default function drawSpectogram(buffer: AudioBuffer, context: AudioContext, canvas: HTMLCanvasElement) {
  const width = 1024
  const height = 400
  const gainNode = context.createGain ? context.createGain() : (context as any).createGainNode()
  ...
  osci.start ? osci.start(0) : (osci as any).noteOn(0)
